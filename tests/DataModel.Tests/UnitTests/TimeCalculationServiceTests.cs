...
{
    private readonly ITimeCalculationService service = new TimeCalculationService();

    [Test]
    public void DurationMinutes_SimpleShift_SubtractsBreak()
    {
        var entry = Entry(start: new(9, 0), end: new(17, 0), breakMinutes: 30);
        service.DurationMinutes(entry).Should().Be(450); // 8h - 30m
    }
    ...
    public void DurationMinutes_PastMidnight_AddsADay()
    {
        ...
        service.DurationMinutes(entry).Should().Be(480); // 8h
    }
    ...
    public void DecimalHours_RoundsToJobPrecision()
    {
        ...
        service.DecimalHours(entry, decimalPlaces: 2).Should().Be(8.25m);
        service.DecimalHours(entry, decimalPlaces: 1).Should().Be(8.3m);
    }
    ...
    public void HoursMinutes_FormatsAsHColonMM()
    {
        ...
        service.HoursMinutes(entry).Should().Be("7:30");
    }
    ...
    public void DurationMinutes_NonPositive_ReturnsZero()
    {
        ...
        service.DurationMinutes(entry).Should().Be(0);
    }
